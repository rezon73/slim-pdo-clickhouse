...
class SelectTest extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->subject = new Statement\Select($this->createMock(PDOClickHouse::class));
    }
    ...
    {
        $this->subject
            ->columns(['sub' => (new Statement\Select($this->createMock(PDOClickHouse::class)))->from('test2')])
            ->from('test1');
    ...
    {
        $this->subject
            ->from(['sub' => (new Statement\Select($this->createMock(PDOClickHouse::class)))->from('test')]);
    ...
    public function testGetValuesWithUnion()
    {
        $this->subject
            ...
            ->from('test1')
            ->union(
                (new Statement\Select($this->createMock(PDOClickHouse::class)))
                    ->columns(['id', 'name'])
                    ->from('test2')
            );
